...
blm0 <- brm(form_full, data = CPUE_df %>% filter(Series=='obs_CPUE0') %>% mutate(Year = factor(Year)),cores=4) 
blm1 <- brm(form_full, data = CPUE_df %>% filter(Series=='obs_CPUE1') %>% mutate(Year = factor(Year)),cores=4) 
blm2 <- brm(form_full, data = CPUE_df %>% filter(Series=='obs_CPUE2') %>% mutate(Year = factor(Year)),cores=4) 

blm0b <- brm(form_sub, data = CPUE_df %>% filter(Series=='obs_CPUE0') %>% mutate(Year = factor(Year)),cores=4) 
blm0b <- brm(form_sub, data = CPUE_df %>% filter(Series=='obs_CPUE1') %>% mutate(Year = factor(Year)),cores=4) 
blm0b <- brm(form_sub, data = CPUE_df %>% filter(Series=='obs_CPUE2') %>% mutate(Year = factor(Year)),cores=4) 
...
agg_CPUE_df_bayes$est_CPUE       <- c(
  exp(posterior_summary(posterior_epred(blm0, newdata = agg_CPUE_df_bayes %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,1]),
  exp(posterior_summary(posterior_epred(blm1, newdata = agg_CPUE_df_bayes %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,1]),
  exp(posterior_summary(posterior_epred(blm2, newdata = agg_CPUE_df_bayes %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,1]))
agg_CPUE_df_bayes$est_CPUE_lower <- c(
  exp(posterior_summary(posterior_epred(blm0, newdata = agg_CPUE_df_bayes %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,3]),
  exp(posterior_summary(posterior_epred(blm1, newdata = agg_CPUE_df_bayes %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,3]),
  exp(posterior_summary(posterior_epred(blm2, newdata = agg_CPUE_df_bayes %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,3]))
agg_CPUE_df_bayes$est_CPUE_upper <- c(
  exp(posterior_summary(posterior_epred(blm0, newdata = agg_CPUE_df_bayes %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,4]),
  exp(posterior_summary(posterior_epred(blm1, newdata = agg_CPUE_df_bayes %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,4]),
  exp(posterior_summary(posterior_epred(blm2, newdata = agg_CPUE_df_bayes %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,4]))
agg_CPUE_df_bayes$model = "Bayes_full"

agg_CPUE_df_bayes_sub <- agg_CPUE_df
agg_CPUE_df_bayes_sub$est_CPUE <- c(
  exp(posterior_summary(posterior_epred(blm0b, newdata = agg_CPUE_df_bayes_sub %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,1]),
  exp(posterior_summary(posterior_epred(blm1b, newdata = agg_CPUE_df_bayes_sub %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,1]),
  exp(posterior_summary(posterior_epred(blm2b, newdata = agg_CPUE_df_bayes_sub %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,1]))
agg_CPUE_df_bayes_sub$est_CPUE_lower <- c(
  exp(posterior_summary(posterior_epred(blm0b, newdata = agg_CPUE_df_bayes_sub %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,3]),
  exp(posterior_summary(posterior_epred(blm1b, newdata = agg_CPUE_df_bayes_sub %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,3]),
  exp(posterior_summary(posterior_epred(blm2b, newdata = agg_CPUE_df_bayes_sub %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,3]))
agg_CPUE_df_bayes_sub$est_CPUE_upper <- c(
  exp(posterior_summary(posterior_epred(blm0b, newdata = agg_CPUE_df_bayes_sub %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,4]),
  exp(posterior_summary(posterior_epred(blm1b, newdata = agg_CPUE_df_bayes_sub %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,4]),
  exp(posterior_summary(posterior_epred(blm2b, newdata = agg_CPUE_df_bayes_sub %>% mutate(X2=0,cvar=0 , Operation=levels(CPUE_df$Operation)[1]), re_formula=NA))[,4]))
